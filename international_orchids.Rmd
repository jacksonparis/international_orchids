...
# DONE: Visualize locations of orchids on a 3D globe -- could use coordinate points from IUCN data, or aggregate the orchids into groups based on their threatened level in each country (scroll over a country to see how many IUCN near threatened, threatened, endangered, and critical orchids grow there).
...
# Alternatively, could make a bipartite network showing connections between the contries

# MAKE A WORDMAP: Search "rationale" column in IUCN assessments data (now part of all_table) for keywords or phrases such as "habitat decline," "threat from agriculture" to show trends in the cause of species decline. 
...
# RANDOM SELECTION: find a random orchid, photo, and picture.

# I think the information in IUCN columns is in html. If it's possible to read it in html form into the shiny app, the extra characters might disappear. 
...
# Now that you have the rows together, you can display each of them 
# in a shiny textOutput. But how to get the search_word function to 
# output a list of this info dataframe, as well as the "ima" image? 

# as.list for each df
# make a dataframe with each of these lists as a column
# With that as the output, you can work with the data separately in 
# Each shiny output that you make 
...
search_word <- function() {
    ...
    random <- sample_n(im_index, size = 1)  
    
    word <- as.character(random[1,8])
    ...
    ima_list <- as.list(ima)
    
    output_list <- combine(info_list, ima_list)
